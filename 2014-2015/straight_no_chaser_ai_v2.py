...
def move_getter(paddle_frect, other_paddle_frect, ball_frect, table_size):
 ...
 y   v
	...
	# TODO - only keep track of opponent velocity in y direction, no need for x
	move_getter.opponent_vel = get_vel(move_getter.previous_opponent_pos, other_paddle_frect.pos)
    move_getter.previous_ball_pos = ball_frect.pos
    move_getter.previous_opponent_pos = other_paddle_frect.pos
    ...
    if not moving_away:
	 move_getter.target_y = get_ball_trajectory(move_getter.ball_vel, ball_frect.pos, paddle_edge, table_size[1], 0)['position'] + ball_frect.size[1]/2
    else:
	...
	# TODO - go after opponent return trajectory, not opponent
	collision_opponent_y = other_paddle_frect.pos[1] + move_getter.opponent_vel[1]*trajectory['time'] + other_paddle_frect.size[1]/2
	if collision_opponent_y < 0:
	    collision_opponent_y = 0
	elif collision_opponent_y > table_size[1]:
	    collision_opponent_y = table_size[1]
	move_getter.target_y = collision_opponent_y
...
def get_ball_trajectory(ball_vel, ball_pos, paddle_edge, table_height, time):
	...
	return {'position':projected_y, 'time':time_to_edge + time}
